...
namespace AspNetCore.Proxy
{
    internal static class WsExtensions
    {
        ...
        internal static async Task ExecuteWsProxyOperationAsync(
            this HttpContext context,
            WsProxy wsProxy
        )
        {
            var uri = await context
                .GetEndpointFromComputerAsync(wsProxy.EndpointComputer)
            ...
            try
            {
                if (!context.WebSockets.IsWebSocketRequest)
                    throw new InvalidOperationException(
                        "An HTTP request cannot be routed as a WebSocket proxy operation."
                    );

                if (!uri.StartsWith("ws", StringComparison.OrdinalIgnoreCase))
                    throw new InvalidOperationException(
                        "Only forwarded addresses starting with 'ws://' or 'ws://' are supported for WebSocket requests."
                    );
                ...
                foreach (var headerEntry in context.Request.Headers)
                {
                    if (
                        !Helpers
                            .WebSocketNotForwardedHeaders
                            .Contains(headerEntry.Key, StringComparer.OrdinalIgnoreCase)
                    )
                        socketToEndpoint
                            .Options
                ...
                if (options?.BeforeConnect != null)
                    await options
                        .BeforeConnect(context, socketToEndpoint.Options)
                        .ConfigureAwait(false);

                await socketToEndpoint
                    .ConnectAsync(new Uri(uri), context.RequestAborted)
                    .ConfigureAwait(false);

                using var socketToClient = await context
                    .WebSockets
                    .AcceptWebSocketAsync(socketToEndpoint.SubProtocol)
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            while (true)
            {
                ...
                {
                    var closeMessageBytes = Encoding
                        .UTF8
                        .GetBytes($"WebSocket failure.\n\n{e.Message}\n\n{e.StackTrace}");
                    var closeMessage = Encoding
                        .UTF8
                        .GetString(
                            closeMessageBytes,
                            0,
                            Math.Min(closeMessageBytes.Length, CloseMessageMaxSize)
                        );
                    await destination
                        .CloseOutputAsync(
